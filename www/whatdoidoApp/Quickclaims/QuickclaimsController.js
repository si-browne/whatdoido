angular.module("QuickclaimsModule").controller("QuickclaimsController", function ($scope, $window) {
  ...
  $scope.findPolicy = function(key){
    ...
    for (i = 0; i < lengthofpolicyarray;i++)
    {
      ...
      if(key == $scope.insurance_policy[i].policy_id)
      {
        ...
        //store array item
        $scope.selectedInsCompItem = $scope.car_insurance_company[$scope.compIdVar];
  ...
  $scope.displaySingleClaim = function(claim_id){
    ...
    for (i = 0; i < $scope.car_insurance_claim.length; i++)
    {
      if($scope.car_insurance_claim[i].claim_id == claim_id)
      {
        ...
        return $scope.singleClaim = $scope.car_insurance_claim[i];
      }
    }
  };
    ...
    if (deleteConfirm == true){
      ...
      for (i = 0; i < $scope.selectedCarInsClaims.length; i++)
      {
        ...
        if($scope.car_insurance_claim[i].claim_id == claim_id)
        {
          ...
          //interestingly here, breaks it if i use the original car_insurance_claim array
  ...
  $scope.startClaim = function () {
    ...
    var addPerson = {
      ...
      person_id: $scope.claim_person.length++,
      claim_id: $scope.car_insurance_claim.length++,
      is_driver: $scope.isdriverOwner,
      //witness temporarily false, until stage 5
      is_witness: false,
    ...
    var addClaim = {
      ...
      //++ to compensate for array indexing earlier.
      company_id: $scope.compIdVar + 1,
    ...
    //claim title and if the user is to blame for the accident
    $scope.claimTitleRecorded;
    $scope.areyouBlame;

  $scope.claim_person.push(addPerson);
  //building variable
  //$scope.car_insurance_claim.push(addClaimStage1);
  ...
  $scope.recommendationSystem = function () {

    //if x amount of variables, maybe case statements.
  };

  $scope.showStage7 = function () {
    $scope.showStg1 = false;
    $scope.showStg2 = false;
    $scope.showStg3 = false;
    $scope.showStg4 = false;
